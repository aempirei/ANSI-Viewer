...
main()
{
    ...
    const unsigned char extended_map[] = {
        176, 177, 178, 219, 220, 221, 222, 223, 254, 250, 0
    };
    ...
    fprintf(stderr, "' ansi size : 80x%u (%u bytes)\n\n", maxypos, 80 * maxypos * 2);

    for (i = 0; i < colortable_siz; i++) {

        fprintf(stderr, "long %%%%%s_%s_%s_%s",
                ansi[colortable[i][0]],
                ansi[colortable[i][1]], ansi[colortable[i][0]], ansi[colortable[i][1]]);

        fprintf(stderr, ", %%%%%s_%s_%s_%s",
                ansi[colortable[i][0]],
                ansi[colortable[i][0]], ansi[colortable[i][1]], ansi[colortable[i][1]]);

        fprintf(stderr, " ' %02i. %s on %s\n", i, ansi_name[colortable[i][0]],
                ansi_name[colortable[i][1]]);
    }
